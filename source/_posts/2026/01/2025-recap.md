...
結果意料之中地慢，因為一開始的程式設計原本就只是針對「圖片」而非「影片」服務的，速度上根本達不到該有的需求。為解決這件事，就要請出我們的新工作站 GPU。把 tile matching 的 metric 換成純粹的圖片像速差值運算後，就可以調用 pytorch 下去處理這件事。最終在 2x 5070 Ti 的加速下，可以用 10 分鐘左右算出來最終想要的成果。
